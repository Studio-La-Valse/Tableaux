...
namespace Tableaux.Models.Engine
{
    public class KeyPointer
    {
        ...
        public ColorHSB Color { get; private set; }
        ...
        private void UpdateColorHSB(Pointer pointer)
        {
            var hue = MathUtils.Map(Angle, 0, Math.PI * 2, 0, ColorHSB.MaxValue);
            var sat = MathUtils.Map(InteralPosition.DistanceTo(new XY(0, 0)), 0, 1, 0, ColorHSB.MaxValue);
            var bri = pointer.Color.Brightness;

            Color = new ColorHSB(hue, sat, bri);
        }
    }
}
