...
export default class DeconstructChannelState extends GraphNode {
  ...
  constructor(modelId: string) {
    ...
    this.outputNoteVelocities = this.registerObjectOutput<{
      note: number
      velocity: number
    }>('Note Velocities')
    this.outputKeyPressures = this.registerObjectOutput<{
      key: number
      pressure: number
    }>('Key Pressures')
    this.outputControllerValues = this.registerObjectOutput<{
      controller: number
      value: number
    }>('Controller Values')
  ...
  protected async solve(inputIterators: InputIteratorsAsync): Promise<void> {
    inputIterators.singletonOnly(this.inputState).forEach((state) => {
