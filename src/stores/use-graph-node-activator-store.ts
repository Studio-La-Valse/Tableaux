...
export const useGraphNodeActivatorStore = defineStore(
  'graph-node-activator-store',
  () => {
    const activatorTree = new ActivatorGroup('root');

    function register(
      path: string[],
      activate: (id: string, path: string[]) => GraphNode
    ) {
      ...
      if (!tree.findActivator(path[path.length - 1])) {
        tree.activators.push(
          new Activator(path[path.length - 1], (id) => activate(id, path))
        );
      }
    }
