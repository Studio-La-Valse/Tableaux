          ...
          <div class="io-card">
            ...
            <div v-for="(input, idx) in inputs" :key="idx" class="io-row">
              ...
              <select v-model="input.type" :disabled="mode === 'edit'">
                <option v-for="type in IOTypes" :key="type" :value="type">{{ type }}</option>
              </select>
              <button v-if="mode === 'create'" class="remove-btn" @click="removeInput(idx)">
                🗑️
              </button>
            </div>
            <button v-if="mode === 'create'" class="add-btn" @click="addInput">+ Add Input</button>
          </div>
...
<script lang="ts" setup>
  ...
  const errors = computed(() => {
    ...
    const { sanitized: parts, errors } = validateFullNodePath(nodeName.value);
    for (const error of errors) {
      list.push(`Invalid path part: ${error}`);
    }

    if (graphNodeRegistry.getDefinition(parts)) {
      list.push('Node with this path is already registered.');
    }
