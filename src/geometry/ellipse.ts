...
export function createUnitEllipse(transformation: TransformationMatrix = identity()): Ellipse {
  return { kind: 'ellipse', transformation }
}
...
export function deconstruct(ellipse: Ellipse | Circle): DeconstructedEllipse {
  ...
  const radiusY = Math.hypot(transformedY.x - origin.x, transformedY.y - origin.y)
...
export function scaleUniform(ellipse: Ellipse, origin: XY, factor: number): Ellipse {
